...
public class Streams {
    ...
    public static void main(String[] args) {
        ...
        List<String> stringList =
                Lists.newArrayList("ddd2", "aaa2", "bbb1", "aaa1", "bbb3", "ccc", "bbb2", "ddd1");

        // Java 8中的Collections类的功能已经有所增强，
        // 你可以直接通过调用Collections.stream()
        ...
        stringList.stream().filter(s -> s.startsWith("a")).forEach(s -> log.info("s: {}", s));
        //aaa2 aaa1

        //Sorted是一个中间操作，能够返回一个排过序的流对象的视图。
        //流对象中的元素会默认按照自然顺序进行排序，除非你自己指定一个Comparator接口来改变排序规则。
        stringList.stream().sorted().filter(s -> s.startsWith("a")).forEach(s -> log.info("s: {}", s));
        //aaa1 aaa2

        log.info("stringList: {}", stringList);
        //[ddd2, aaa2, bbb1, aaa1, bbb3, ccc, bbb2, ddd1]

        // map是一个对于流对象的中间操作，通过给定的方法，能够把流对象中的每一个元素对应到另外一个对象上。
        ...
        log.info("startWithB: {}", startWithB);
        //3
        ...
        reduced.ifPresent(s -> log.info("s: {}", s));
